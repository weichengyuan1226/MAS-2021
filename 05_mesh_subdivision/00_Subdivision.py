...
from mysubdivision import mesh_subdivide_pyramid

unload_modules('shapes')
from shapes import Sphere, Cylinder, Torus
...
artist6 = MeshArtist(subd, layer="04_subdivide_loop")
artist6.clear_layer()
...
artist7 = MeshArtist(subd, layer="05_subdivide_doosabin")
artist7.clear_layer()
...
artist9 = MeshArtist(mesh5, layer="06_3d")
artist9.clear_layer()
artist9.draw_mesh()

#3d mesh - Custom shapes
my_circle = Circle([[0, 0, 0], [0, 1, 0]], 10)
my_cone = Cone(my_circle, 10)
vs, fs = my_cone.to_vertices_and_faces(u=11,v=11)
my_cone_mesh = Mesh.from_vertices_and_faces(vs, fs)

artist = MeshArtist(my_cone_mesh, layer="07_cone")
artist.clear_layer()
artist.draw_mesh()
mesh = my_cone_mesh
...
artist9 = MeshArtist(sd, layer="08_cone_tapered")
artist9.clear_layer()
artist9.draw_mesh()
...
atomium = meshes_join([all_cylinders, spheres_mesh, torus_mesh])

point = [20,0.5,0]
T = Translation.from_vector(point)
atomium.transform(T)

artist10 = MeshArtist(atomium, layer="09 atomium")
